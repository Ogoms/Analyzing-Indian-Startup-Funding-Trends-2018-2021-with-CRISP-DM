...
## Introduction
This project aims to analyze Indian startup funding trends using the CRISP-DM (Cross-Industry Standard Process for Data Mining) methodology. It involves connecting to a database, querying relevant data, and performing exploratory data analysis (EDA) using Python and various data analysis libraries.
...
The project connects to a SQL Server database using pyodbc and environment variables stored in a .env file for security. The connection string is dynamically created using the credentials obtained from the .env file.

## Data Retrieval
- Data from the years 2020 and 2021 is retrieved from the database tables 'LP1_startup_funding2020' and 'LP1_startup_funding2021' respectively.
- Additionally, data from the years 2018 and 2019 is loaded from CSV files ('startup_funding2018.csv' and 'startup_funding2019.csv').

## Analysis
- Data preprocessing and exploratory data analysis (EDA) are performed using Pandas.
- Visualization of funding trends and statistical analysis are conducted using Matplotlib, Seaborn, and other relevant libraries.
...
Reporting: Findings from the analysis are conveniently accessible through a published report on Power BI, offering stakeholders an intuitive platform to explore and interpret the insights derived from the analysis. You can access the report via the following link: https://app.powerbi.com/groups/me/reports/94dbde38-bc80-4e14-88e6-ec894271874d/244268b0cbd879e70d70?experience=power-bi.
